...
int test_namespace_validation(void)
{
	...
	ASSERT(conn->err == MONGO_NS_INVALID);
	ASSERT(strncmp(conn->errstr, "Database name may not contain", 28)
	       == 0);
	mongo_clear_errors(conn);
	...
	ASSERT(conn->err == MONGO_NS_INVALID);
	ASSERT(strncmp(conn->errstr, "Collection may not contain '$'", 29)
	       == 0);
	mongo_clear_errors(conn);
	...
	ASSERT(mongo_validate_ns(conn, "test.fo.o.") == MONGO_ERROR);
	ASSERT(conn->err == MONGO_NS_INVALID);
	ASSERT(strncmp(conn->errstr, "Collection may not end with '.'", 30)
	       == 0);
	mongo_clear_errors(conn);
	...
	ASSERT(strncmp
	       (conn->errstr,
		"Namespace too long; has 129 but must <= 128.", 32) == 0);
	mongo_clear_errors(conn);
...
int test_insert_limits(void)
{
	...
	ASSERT(mongo_insert_batch
	       (conn, "test.foo", (const bson **) objs, 2, NULL,
		0) == MONGO_ERROR);
	ASSERT(conn->err == MONGO_BSON_TOO_LARGE);
...
int test_get_last_error_commands(void)
{
	...
	ASSERT(strcmp((const char *) conn->lasterrstr, "forced error") ==
	       0);
