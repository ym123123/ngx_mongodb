...
void test_invalid_structure(void)
{
	...
	test_bson_from_bcon(bc_incomplete, BCON_DOCUMENT_INCOMPLETE,
			    BSON_VALID);
}
...
void test_problematic_structure(void)
{
	...
	test_bson_from_bcon(bc_incomplete, BCON_DOCUMENT_INCOMPLETE,
			    BSON_VALID);
	bcon bc_bracket_brace[] =
	    { "k0", "v0", "k1", "{", "k11", "v11", "]", "v12", "}", BEND };
	test_bson_from_bcon(bc_bracket_brace, BCON_OK, BSON_VALID);	/* key for now */
	bcon bc_brace_bracket[] =
	    { "k0", "v0", "k1", "[", "k11", "v11", "}", "]", BEND };
...
void test_valid_structure(void)
{
	...
	bcon bc_embedded[] =
	    { "k0", "v0", "k1", "{", "k10", "v10", "k11", "v11", "}", "k2",
   "v2", BEND };
	test_bson_from_bcon(bc_embedded, BCON_OK, BSON_VALID);
	bcon bc_embedded_2[] =
	    { "k0", "v0", "k1", "{", "k10", "v10", "k11", "{", "k110",
 "v110", "}", "k12", "v12", "}", "k2", "v2", BEND };
...
void test_oid_generation(void)
{
	...
	char **poid_s = (char **) &oid_s;
	bcon oid_bc[] =
	    { "_id", BO(""), "user_id", BRO(oid_s), "admin_id",
	BPO(&poid_s), BEND };
...
void test_example_awesome(void)
{
	...
	bcon awesome[] =
	    { "BSON", "[", "awesome", BF(5.05), BI(1986), "]", BEND };
...
int main(int argc, char **argv)
{
	...
	if (argc > 1)
		verbose = 1;
	for (i = 0; i < sizeof(test_suite) / sizeof(struct test_suite);
	     i++) {
		if (verbose)
			printf("%s:\n", test_suite[i].name);
		(*test_suite[i].fn) ();
