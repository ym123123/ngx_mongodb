...
void json_to_bson_append_element(bson_buffer * bb, const char *k,
...
void json_to_bson_append_array(bson_buffer * bb, struct json_object *a)
{
	...
	for (i = 0; i < json_object_array_length(a); i++) {
		sprintf(buf, "%d", i);
		json_to_bson_append_element(bb, buf,
					    json_object_array_get_idx(a,
								      i));
	}
}
...
char *json_to_bson(char *js)
{
	...
	if (!json_object_is_type(o, json_type_object)) {
		fprintf(stderr,
			"json_to_bson needs a JSON object, not type\n");
		return 0;
	}
...
int json_to_bson_test(char *js, int size, const char *hash)
{
	...
	if (size != bson_size(&b)) {
		fprintf(stderr,
			"sizes don't match [%s] want != got %d != %d\n",
			js, size, bson_size(&b));
		bson_destroy(&b);
		return 0;
	}
	...
	mongo_md5_init(&st);
	mongo_md5_append(&st, (const mongo_md5_byte_t *) b.data,
			 bson_size(&b));
...
int main()
{
	...
	JSONBSONTEST("{ 'x' : true }", 9,
		     "6fe24623e4efc5cf07f027f9c66b5456");
	JSONBSONTEST("{ 'x' : null }", 8,
		     "12d43430ff6729af501faf0638e68888");
	JSONBSONTEST("{ 'x' : 5.2 }", 16,
		     "aaeeac4a58e9c30eec6b0b0319d0dff2");
	JSONBSONTEST("{ 'x' : 'eliot' }", 18,
		     "331a3b8b7cbbe0706c80acdb45d4ebbe");
	JSONBSONTEST("{ 'x' : 5.2 , 'y' : 'truth' , 'z' : 1.1 }", 40,
		     "7c77b3a6e63e2f988ede92624409da58");
	JSONBSONTEST("{ 'x' : 4 }", 12,
		     "d1ed8dbf79b78fa215e2ded74548d89d");
	JSONBSONTEST("{ 'x' : 5.2 , 'y' : 'truth' , 'z' : 1 }", 36,
		     "8993953de080e9d4ef449d18211ef88a");
	JSONBSONTEST("{ 'x' : 'eliot' , 'y' : true , 'z' : 1 }", 29,
		     "24e79c12e6c746966b123310cb1a3290");
	JSONBSONTEST("{ 'a' : { 'b' : 1.1 } }", 24,
		     "31887a4b9d55cd9f17752d6a8a45d51f");
	JSONBSONTEST
	    ("{ 'x' : 5.2 , 'y' : { 'a' : 'eliot' , 'b' : true } , 'z' : null }",
