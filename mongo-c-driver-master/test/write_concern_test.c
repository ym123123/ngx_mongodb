...
void create_capped_collection(mongo * conn)
{
	...
	mongo_create_capped_collection(conn, "test", "wc", 1000000, 0,
				       NULL);
}
...
void test_batch_insert_with_continue(mongo * conn)
{
	...
	mongo_create_simple_index(conn, TEST_NS, "n", MONGO_INDEX_UNIQUE,
				  NULL);
	...
	ASSERT(mongo_count(conn, TEST_DB, TEST_COL, bson_empty(&empty)) ==
	       9);
...
void test_update_and_remove(mongo * conn)
{
	...
	ASSERT(mongo_find_one
	       (conn, "test.wc", query, bson_empty(&empty),
		NULL) == MONGO_OK);
	...
	ASSERT(mongo_find_one
	       (conn, "test.wc", query, bson_empty(&empty),
		NULL) == MONGO_OK);
	ASSERT(mongo_update(conn, "test.wc", query, update, 0, NULL) ==
	       MONGO_OK);
	ASSERT(mongo_find_one
	       (conn, "test.wc", query, bson_empty(&empty),
		NULL) == MONGO_OK);
	...
	ASSERT(mongo_update(conn, "test.wc", query, update, 0, wc) ==
	       MONGO_ERROR);
	ASSERT(conn->err == MONGO_WRITE_ERROR);
	ASSERT_EQUAL_STRINGS(conn->lasterrstr,
...
void test_insert(mongo * conn)
{
	...
	ASSERT(mongo_remove(conn, TEST_NS, bson_empty(empty), wc1) ==
	       MONGO_OK);
	...
	ASSERT(mongo_count(conn, TEST_DB, TEST_COL, bson_empty(empty)) ==
	       1);
	ASSERT(mongo_insert_batch
	       (conn, TEST_NS, (const bson **) objs, 2, NULL,
		0) == MONGO_OK);
	ASSERT(mongo_count(conn, TEST_DB, TEST_COL, bson_empty(empty)) ==
	       3);
	...
	mongo_clear_errors(conn);
	ASSERT(mongo_insert_batch
	       (conn, TEST_NS, (const bson **) objs, 2, wc1,
		0) == MONGO_ERROR);
	ASSERT(conn->err == MONGO_WRITE_ERROR);
	ASSERT_EQUAL_STRINGS(conn->errstr,
			     "See conn->lasterrstr for details.");
