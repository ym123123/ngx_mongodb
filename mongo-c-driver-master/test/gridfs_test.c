...
void test_gridfile(gridfs * gfs, char *data_before, int64_t length,
		   char *filename, char *content_type)
{
	...
	n = 0;
	while (i > INT_MAX) {
		mongo_md5_append(pms,
				 (const mongo_md5_byte_t *) data_before +
				 (n * INT_MAX), INT_MAX);
		i -= INT_MAX;
		n += 1;
	}
...
void test_basic(void)
{
	...
	for (i = LOWER; i <= UPPER; i += DELTA) {
		...
		gridfs_store_buffer(gfs, data_before, i, "input-buffer",
				    "text/html");
		test_gridfile(gfs, data_before, i, "input-buffer",
			      "text/html");
		...
		fclose(fd);
		gridfs_store_file(gfs, "input-file", "input-file",
				  "text/html");
		test_gridfile(gfs, data_before, i, "input-file",
			      "text/html");
	}
...
void test_large(void)
{
	...
	ASSERT(strcmp(gridfile_get_filename(gfile), "bigfile-stream") ==
	       0);
	ASSERT(gridfile_get_contentlength(gfile) == filesize);
