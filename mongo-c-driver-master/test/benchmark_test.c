...
static void make_large(bson * out, int i)
{
	...
	bson_append_new_oid(out, "_id");
	bson_append_int(out, "x", i);
	bson_append_string(out, "base_url",
			   "http://www.example.com/test-me");
	...
	bson_append_string(out, "description",
			   "i am a long description string");
	bson_append_string(out, "author", "Holly Man");
	bson_append_string(out, "dynamically_created_meta_tag",
			   "who know\n what");
	bson_append_finish_object(out);
...
static void index_insert_small_test(void)
{
	int i;
	bson b;
	ASSERT(mongo_create_simple_index
	       (conn, DB ".index.small", "x", 0, NULL) == MONGO_OK);
	for (i = 0; i < PER_TRIAL; i++) {
		make_small(&b, i);
...
static void index_insert_medium_test(void)
{
	...
	bson b;
	ASSERT(mongo_create_simple_index
	       (conn, DB ".index.medium", "x", 0, NULL) == MONGO_OK);
	for (i = 0; i < PER_TRIAL; i++) {
		make_medium(&b, i);
...
static void index_insert_large_test(void)
{
	...
	bson b;
	ASSERT(mongo_create_simple_index
	       (conn, DB ".index.large", "x", 0, NULL) == MONGO_OK);
	for (i = 0; i < PER_TRIAL; i++) {
		make_large(&b, i);
...
static void batch_insert_medium_test(void)
{
	...
	for (i = 0; i < (PER_TRIAL / BATCH_SIZE); i++) {
		...
		mongo_insert_batch(conn, DB ".batch.medium", bp,
				   BATCH_SIZE, NULL);
