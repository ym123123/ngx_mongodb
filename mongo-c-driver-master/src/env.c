...
int mongo_env_close_socket(mongo *conn)
{
	close_func(conn);
	if (conn->sock)
...
int mongo_env_write_socket(mongo * conn, const void *buf, size_t len)
{
	...
	while (len) {
		...
		if (sent == -1) {
			if (errno == EPIPE)
				conn->connected = 0;
			__mongo_set_error(conn, MONGO_IO_ERROR,
...
int mongo_env_set_socket_op_timeout(mongo * conn, int millis)
{
	...
	if (setsockopt
	    (conn->sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) == -1) {
		conn->err = MONGO_IO_ERROR;
		__mongo_set_error(conn, MONGO_IO_ERROR,
				  "setsockopt SO_RCVTIMEO failed.", errno);
		return MONGO_ERROR;
	}

	if (setsockopt
	    (conn->sock, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)) == -1) {
		__mongo_set_error(conn, MONGO_IO_ERROR,
				  "setsockopt SO_SNDTIMEO failed.", errno);
		return MONGO_ERROR;
	}
					 ...
					 const char *sock_path)
{
	...
	fcntl(conn->sock, F_SETFL,
	      fcntl(conn->sock, F_GETFL) | O_NONBLOCK);
...
int mongo_env_socket_connect(mongo * conn, const char *host, int port)
{
	...
	if (status != 0) {
		bson_errprintf("getaddrinfo failed: %s",
			       gai_strerror(status));
	...
	for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
		...
		fcntl(conn->sock, F_SETFL,
		      fcntl(conn->sock, F_GETFL) | O_NONBLOCK);
		status =
		    connect(conn->sock, ai_ptr->ai_addr,
			    ai_ptr->ai_addrlen);
		if (status != 0) {
//            mongo_env_close_socket( conn->sock );
//            conn->sock = 0;
//            continue;
		...
		if (ai_ptr->ai_protocol == IPPROTO_TCP) {
			...
			if (conn->op_timeout_ms > 0)
				mongo_env_set_socket_op_timeout(conn,
								conn->
								op_timeout_ms);
		}
