...
MONGO_EXPORT void bson_oid_from_string(bson_oid_t * oid, const char *str)
{
	...
	for (i = 0; i < 12; i++) {
		oid->bytes[i] =
		    (hexbyte(str[2 * i]) << 4) | hexbyte(str[2 * i + 1]);
	}
}
...
MONGO_EXPORT void bson_print_raw(const char *data, int depth)
{
	...
	while (bson_iterator_next(&i)) {
		...
		case BSON_CODEWSCOPE:
			bson_printf("BSON_CODE_W_SCOPE: %s",
				    bson_iterator_code(&i));
...
MONGO_EXPORT bson_type bson_find(bson_iterator * it, const bson * obj,
				 const char *name)
{
...
MONGO_EXPORT bson_timestamp_t bson_iterator_timestamp(const bson_iterator *
						      i)
{
	bson_timestamp_t ts;
					   ...
					   bson * scope)
{
	if (bson_iterator_type(i) == BSON_CODEWSCOPE) {
		int code_len;
		bson_little_endian32(&code_len,
				     bson_iterator_value(i) + 4);
		bson_init_data(scope,
			       (void *) (bson_iterator_value(i) + 8 +
					 code_len));
		_bson_reset(scope);
		scope->finished = 1;
	} else {
...
MONGO_EXPORT int bson_iterator_bin_len(const bson_iterator * i)
{
	return (bson_iterator_bin_type(i) == BSON_BIN_BINARY_OLD)
	    ? bson_iterator_int_raw(i) - 4 : bson_iterator_int_raw(i);
}
...
{
	return (bson_iterator_bin_type(i) == BSON_BIN_BINARY_OLD)
	    ? bson_iterator_value(i) + 9 : bson_iterator_value(i) + 5;
}
...
static int bson_append_estart(bson * b, int type, const char *name,
...
MONGO_EXPORT int bson_append_long(bson * b, const char *name,
				  const int64_t i)
{
...
MONGO_EXPORT int bson_append_double(bson * b, const char *name,
				    const double d)
{
...
MONGO_EXPORT int bson_append_bool(bson * b, const char *name,
				  const bson_bool_t i)
{
				   ...
				   bson_type type)
{
	...
	if (bson_check_string(b, (const char *) value, sl - 1) ==
	    BSON_ERROR)
		return BSON_ERROR;
...
MONGO_EXPORT int bson_append_string(bson * b, const char *name,
				    const char *value)
{
	return bson_append_string_base(b, name, value, strlen(value),
				       BSON_STRING);
}

MONGO_EXPORT int bson_append_symbol(bson * b, const char *name,
				    const char *value)
{
	return bson_append_string_base(b, name, value, strlen(value),
				       BSON_SYMBOL);
...
MONGO_EXPORT int bson_append_code(bson * b, const char *name,
				  const char *value)
{
	return bson_append_string_base(b, name, value, strlen(value),
				       BSON_CODE);
...
MONGO_EXPORT int bson_append_string_n(bson * b, const char *name,
				      const char *value, size_t len)
{
	return bson_append_string_base(b, name, value, len, BSON_STRING);
}

MONGO_EXPORT int bson_append_symbol_n(bson * b, const char *name,
				      const char *value, size_t len)
{
	return bson_append_string_base(b, name, value, len, BSON_SYMBOL);
}

MONGO_EXPORT int bson_append_code_n(bson * b, const char *name,
				    const char *value, size_t len)
{
					    ...
					    const bson * scope)
{
	...
	if (!scope)
		return BSON_ERROR;
	sl = len + 1;
	if (4 + 4 + (long long) sl + (long long) bson_size(scope) >
	    (long long) INT32_MAX) {
		b->err = BSON_SIZE_OVERFLOW;
		return BSON_ERROR;
	}
	size = 4 + 4 + sl + bson_size(scope);
	if (bson_append_estart(b, BSON_CODEWSCOPE, name, size) ==
	    BSON_ERROR)
		return BSON_ERROR;
...
MONGO_EXPORT int bson_append_code_w_scope(bson * b, const char *name,
					  const char *code,
					  const bson * scope)
{
	return bson_append_code_w_scope_n(b, name, code, strlen(code),
					  scope);
}

MONGO_EXPORT int bson_append_binary(bson * b, const char *name, char type,
				    const char *str, size_t len)
{
	if (type == BSON_BIN_BINARY_OLD) {
		size_t subtwolen = len + 4;
		if (bson_append_estart
		    (b, BSON_BINDATA, name, 4 + 1 + 4 + len) == BSON_ERROR)
			return BSON_ERROR;
		...
		if (bson_append_estart(b, BSON_BINDATA, name, 4 + 1 + len)
		    == BSON_ERROR)
			return BSON_ERROR;
		bson_append32_as_int(b, (int) len);
		bson_append_byte(b, type);
...
MONGO_EXPORT int bson_append_oid(bson * b, const char *name,
...
MONGO_EXPORT int bson_append_regex(bson * b, const char *name,
				   const char *pattern, const char *opts)
{
	const size_t plen = strlen(pattern) + 1;
	const size_t olen = strlen(opts) + 1;
	if (bson_append_estart(b, BSON_REGEX, name, plen + olen) ==
	    BSON_ERROR)
		return BSON_ERROR;
...
MONGO_EXPORT int bson_append_bson(bson * b, const char *name,
				  const bson * bson)
{
	if (!bson)
		return BSON_ERROR;
...
MONGO_EXPORT int bson_append_element(bson * b, const char *name_or_null,
				     const bson_iterator * elem)
{
	...
	if (name_or_null == NULL) {
		...
		size_t data_size =
		    size - 2 - strlen(bson_iterator_key(elem));
		bson_append_estart(b, elem->cur[0], name_or_null,
				   data_size);
...
MONGO_EXPORT int bson_append_timestamp(bson * b, const char *name,
				       bson_timestamp_t * ts)
{
	if (bson_append_estart(b, BSON_TIMESTAMP, name, 8) == BSON_ERROR)
		return BSON_ERROR;
...
MONGO_EXPORT int bson_append_timestamp2(bson * b, const char *name,
...
MONGO_EXPORT int bson_append_date(bson * b, const char *name,
				  bson_date_t millis)
{
	if (bson_append_estart(b, BSON_DATE, name, 8) == BSON_ERROR)
		return BSON_ERROR;
	bson_append64(b, &millis);
	return BSON_OK;
}

MONGO_EXPORT int bson_append_time_t(bson * b, const char *name,
				    time_t secs)
{
...
{
	if (bson_append_estart(b, BSON_OBJECT, name, 5) == BSON_ERROR)
		return BSON_ERROR;
...
{
	if (bson_append_estart(b, BSON_ARRAY, name, 5) == BSON_ERROR)
		return BSON_ERROR;
	b->stack[b->stackPos++] = b->cur - b->data;
	bson_append32(b, &zero);
	return BSON_OK;
