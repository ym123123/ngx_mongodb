...
typedef enum mongo_cursor_error_t {
	...
	MONGO_CURSOR_BSON_ERROR
			     /**< Something is wrong with the BSON provided. See conn->err
                                  for details. */
...
typedef struct mongo_write_concern {
	...
	const char *mode;
		      /**< Either "majority" or a getlasterrormode. Overrides w value. */
...
MONGO_EXPORT double mongo_count(mongo * conn, const char *db,
				const char *coll, const bson * query);
